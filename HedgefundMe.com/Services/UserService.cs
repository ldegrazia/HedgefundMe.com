...
namespace HedgefundMe.com.Services
{
    ...
    public partial class UserService
    {
        ...
        private readonly ProjectEntities _db = new ProjectEntities();
        /// <summary>
        /// Creates a test user
        /// </summary>
        /// <returns></returns>
        public User Create()
        {
            ...
            return new User
            {
                ...
                Password =  Crypto.HashPassword(usr),
        ...
        {
            var pswd = Crypto.HashPassword(password);
        ...
        public User AdminUpdate(EditUser newUser, HttpPostedFileBase userPhoto)
        {
            ...
            {
                usr.Password =  Crypto.HashPassword(newUser.Password);
            }
        ...
        public bool LogOn(string userName, string password)
        {
            try
            {
                //var pswd = Crypto.VerifyHashedPassword(hashedPassword, password) FormsAuthentication.HashPasswordForStoringInConfigFile(password, Constants.HashMethod);
                //&& p.Password == pswd
                User theuser;
                if (userName.Contains(Constants.AtSymbol))
                {
                    theuser = _db.Users.Single(p => p.Email == userName  && p.AppName == _appName);
                }
                else
                {
                    theuser = _db.Users.Where(t => t.AppName == _appName).Single(p => p.UserName == userName);
                }
                if (theuser != null)
                {
                    var doesPasswordMatch = Crypto.VerifyHashedPassword(theuser.Password, password);
        ...
        public bool ChangePassword(string userName, string oldPassword, string newpassword)
        {
            try
            {
                ...
                var oldpswd = Crypto.HashPassword(oldPassword);
                var newpswd = Crypto.HashPassword(newpassword);
